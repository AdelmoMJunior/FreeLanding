...
type WhatsappFloatingProps = Readonly<{
  content: PublicLandingContent;
}>;

function isExternalWhatsappLink(href: string) {
  try {
    const url = new URL(href);

    return url.protocol === "https:" && (url.hostname === "wa.me" || url.hostname.endsWith(".whatsapp.com"));
  } catch {
    return false;
  }
}

export function WhatsappFloating({ content }: WhatsappFloatingProps) {
  const href = content.whatsapp?.href ?? "#contato";
  const isExternal = isExternalWhatsappLink(href);
  const label = content.whatsapp?.label ?? "Ir para a seção de contato";
  const title = content.whatsapp ? "WhatsApp" : "Contato";

  return (
    <a
      href={href}
      className="fixed bottom-[calc(env(safe-area-inset-bottom)+1.25rem)] right-5 z-20 inline-flex size-14 items-center justify-center rounded-full bg-[#128c4a] text-white shadow-2xl shadow-emerald-950/30 motion-safe:transition motion-safe:hover:scale-105 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--brand-color)] sm:bottom-7 sm:right-7"
      aria-label={label}
